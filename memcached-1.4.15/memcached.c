...
static void maxconns_handler(const int fd, const short which, void *arg) {
    struct timeval t = {.tv_sec = 0, .tv_usec = 10000}; // 时间
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (event_add(&c->event, 0) == -1) {
        ...
        /**
            如果失败了, 要将 struct conn 放入到空闲连接数组中,
            conn_add_to_freelist() 会尝试扩大空闲连接数组的大小.
            如果尝试失败将会导致 struct conn 被释放空间.
        */
...
static void conn_close(conn *c) {
    ...
    close(c->sfd);
    pthread_mutex_lock(&conn_lock);
    allow_new_conns = true; // 允许建立新的连接
    pthread_mutex_unlock(&conn_lock);
    conn_cleanup(c);

    /* if the connection has big buffers, just free it */
    // 当这个连接产生了较大的交互数据的时候, 会将 conn 结构体直接释放. 并不是任何时候都将 conn 放到空闲 conn 链表中
 ...
 * Returns 0 on success, -1 on out-of-memory.
 */
 // 关于 iov 查看: http://www.makelinux.net/ldd3/chp-3-sect-7
static int ensure_iov_space(conn *c) {
    assert(c != NULL);
 ...
 * 填充 UDP 头部到将要发出的消息中, 在发送消息的前一步才填充
 */
...
// 将 str 中的数据写入到 conn *c 的内存中, 准备返回给客户端
static void out_string(conn *c, const char *str) {
    size_t len;

    assert(c != NULL);

    if (c->noreply) { // 如果设置了不需要回复
        if (settings.verbose > 1)
...
static void process_bin_get(conn *c) {
    ...
    if (it) {
        ...
        // 如果不需要回复, 更正 conn 状态为 conn_new_cmd, 继续等待新的命令
        if (c->noreply) {
            conn_set_state(c, conn_new_cmd);
        } else {
            if (c->cmd == PROTOCOL_BINARY_CMD_GETK) {
...
static void init_sasl_conn(conn *c) {
    ...
    // SASL:　http://zh.wikipedia.org/zh-cn/%E7%AE%80%E5%8D%95%E8%AE%A4%E8%AF%81%E4%B8%8E%E5%AE%89%E5%85%A8%E5%B1%82
    if (!c->sasl_conn) {
...
static void dispatch_bin_command(conn *c) {
    ...
    switch (c->cmd) {
        ...
        // 当用户明确说明需要结束服务的时候, 会向服务器发起 QUIT 命令. 服务器收到之后, 会将 conn 装态转换为 conn_closing, 最终会调用 conn_close() 或者 conn_cleanup(), 到此这个连接就真正结束了.
...
static enum try_read_result try_read_udp(conn *c) {
    ...
    // UDP 请求最大只能是一个 UDP 限制大小, 即 65535 个字节
    // 大于 8 字节
    if (res > 8) {
    ...
    while (!stop) {
        ...
        case conn_listening:
            ...
            if (settings.maxconns_fast &&
                ...
                // dispatch_conn_new() 在 thread.c 中定义
                // 目的是把当前的工作交给空闲的线程去做
                // conn_new_cmd 代表将要设置的这个连接的状态为等待接受命令状态
        ...
        case conn_nread:
            ...
            if (res > 0) {
                ...
                // 此时可能还没有读完, 保持 conn_nread 状态, 循环继续
                break;
            }
        ...
        case conn_write:
             ...
             *
             * 需要写回执行结果. 数据需要被组装到 msgbuf 中再发送返回
             */
        ...
        case conn_mwrite:
        // 整合了 conn_write 和 conn_mwrite 两种情况
...
// 所有的监听连接都注册了这个函数. 当有新的连接的时候将会调用此函数
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        ...
        // strdup 分配空间并拷贝 settings.inter 的内容并返回
        // ? settings.inter 保存的是主机名 hostname
...
static int server_socket_unix(const char *path, int access_mask) {
    ...
    // 为 listen fd 在 libevent 中注册一个读事件
    // conn_listening 代表将要将要设置连接的状态为正在连接
    ...
    if (ever != NULL) {
        if (strncmp(ever, "1.", 2) == 0) {
            ...
            if ((ever[2] == '1' || ever[2] == '2') && !isdigit(ever[3])) {
                // 检测 libevent 的版本, memcache 对 libevent 有要求
...
int main (int argc, char **argv) {
    ...
    if (maxcore != 0) {
         ...
         */
         // RLIMIT_CORE 内核转存文件的最大长度。(core文件的大小，内核不做限制)
     ...
     * 忽略 SIGPIPE 信号, 如果需要的话可以通过 errno 值来判断
     */
